 ...
 * the time for nothing (for performance reasons). This is the #1 goal: we should not impact our user'
 * application because of this use-case.</li>
 * <li> Not having an outer join causes n+1 requests to the database. This is not a real issue as
 * we have by default a second-level cache. This means on the first HTTP call we do the n+1 requests,
 * but then all authorities come from the cache, so in fact it's much better than doing an outer join
 * (which will get lots of data from the database, for each HTTP call).</li>
 * <li> As this manages user, for security reasons, we'd rather have a DTO layer.</li>
 * </ul>
 * <p>
...
public class UserResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body all user.
     */
    @GetMapping("/user")
   /* public ResponseEntity<List<UserDTO>> getAllUsers(Pageable pageable) {
	    ...
	    System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"+userService.getAuthorities());

		if (result != null&&!result.isEmpty()) {
			return customResponse.success("user found", result);
		} else {
			return customResponse.failure(404, "user Not found");
		}
    }

    
   /* @GetMapping("/user/{id}")
        public List<String> getAuthorities(@PathVariable) {
  ...
  //  @PreAuthorize("hasRole(\"" + AuthoritiesConstants.ADMIN + "\")")
    ...
    /**
     * {@code DELETE /user/:login} : delete the "login" User.
     *
    ...
    public ResponseEntity<Void> deleteUser(@PathVariable Long id) {
        ...
        return null;
        		...
        		//ResponseEntity.noContent().headers(HeaderUtil.createAlert(applicationName,  "A user is deleted with identifier " + login, login)).build();
    }
